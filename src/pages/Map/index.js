...
const MapComponent = () =>{
  ...
  useEffect(()=>{
	...
	//重新加载克里金插值图
	 const trainAndReDrawKriging =(extent,coord,values,lngs,lats,colors)=>{
		if(!coord){
			return;
		}
    console.log("inKriging",true)
      ...
      let variogram=kriging.train(values,lngs,lats,
			...
			//移除已有图层
			if (canvasLayer != null){
				pageStore.map.removeLayer(canvasLayer);
			}
			//创建新图层
			...
			//向map添加图层
			pageStore.map.addLayer(canvasLayer);
  ...
  const iTimer = () => {
		console.log("定时器启动====")
		pageStore.timer = setInterval(() => {
			playPollutantDistribution();
		}, 2500);
	}

  const playPollutantDistribution=()=>{
		console.log("开始播放========")
    let extent = pageStore.extent;
		//TODO 定义裁剪边界，即所选园区范围
		/*let coord = [[
		...
		//监测时间集合
		//let times=['2020-04-22 10','2020-04-22 11','2020-04-22 12','2020-04-22 13']
		//站点监测值各时间段数组的集合
		//let values=[[100,200,300,400],[200,100,300,400],[400,200,100,300],[100,400,200,300]]
    //站点的经度集合
		//let lngs=[[114.190383,114.192921,114.193213,114.190213],[114.190383,114.192921,114.193213,114.190213],[114.190383,114.192921,114.193213,114.190213],[114.190383,114.192921,114.193213,114.190213]]
    //站点的纬度集合
		//let lats=[[30.460976,30.461092,30.460403,30.459898],[30.460976,30.461092,30.460403,30.459898],[30.460976,30.461092,30.460403,30.459898],[30.460976,30.461092,30.460403,30.459898]]
    //园区各时间段监测值对应的颜色集合
    let coord = pageStore.coord;
    let values = pageStore.values;
    let lngs = pageStore.lngs;
    let lats = pageStore.lats;
    let colors = pageStore.colors;
    console.log("colors1==",colors)
    console.log("values==",values)
		trainAndReDrawKriging(extent,coord,values[pageStore.index],lngs[pageStore.index],lats[pageStore.index],colors[pageStore.index]);
    pageStore.index++;
		if(pageStore.index==pageStore.times.length) {
      pageStore.index=0
		}
	}

  const handlePlayClick=()=> {
    console.log("pageStore.onPlay==",pageStore.onPlay)
		//查询污染分布数据
		let param = { pmCode:"NMHC", parkId:"111111",timeStart:"2020-05-12 10",timeEnd:"2020-05-12 14" };
    pageStore.getPollutantDistribution(param);
    let flag = pageStore.onPlay
		if(flag){
			clearInterval(pageStore.timer);
		}else {
			setTimeout(iTimer,0);
		}
    pageStore.onPlay=!flag;
	}
  //切换园区的时候调用
  const handleParkChange=()=>{
    //实际应用中根据选中的园区获取园区中心,注:所有从百度地图获取的坐标必须做一次坐标系转换(transform)，否则定位会有偏差
    let parkCenter = ["114.191771","30.460555"]
    pageStore.map.getView().setCenter(transform(parkCenter,'BD:09','EPSG:3857'));
  }
////////////////////////////////////////↓↓↓↓无视此处代码↓↓↓↓//////////////////////////////////////////////////////////
  ...
  const initmap=(mapCenter)=> {
		...
		addCoordinateTransforms("EPSG:3857", "BD:09",
			function(coordinate) {
    ...
    pageStore.map.on('pointermove',function(e) {
		...
		return (
          ...
          <Button type="primary" onClick={handleParkChange}>切换园区</Button>
					</div>
			</div>
		)
